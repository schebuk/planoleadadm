...
export default {
    ...
    methods: {
      getRegisters(page = 1,url = '/api/segments/?page=') {
        this.tableData.draw++;
      ...
      getColumns(){
        axios.post('/api/config/columns/segments',{id:1})
          .then(response => {
      ...
      save(formfields,modalname){        
        ...
        if(formfields.id){
          ...
          axios.post('/api/segments/edit', values)
          .then(response => {
            this.snackbar = true
          ...
          axios.post('/api/segments/save', values)
          .then(response => {  
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      saveBulk(formfields,modalname){  
        ...
        axios.post('/api/segments/edit/multiple', bodyFormData, {
          headers: {
      ...
      changestatus(status,id){
        status = status? 1:0
        axios.get('/api/segments/status/'+id+'/'+status)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      trash(id){
        axios.get('/api/segments/trash/'+id)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      onFileChanged(e) {
        ...
        axios.post('/api/segments/import', formData)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      exportRegisters(){
        ...
        axios.post('/api/segments/export',{responseType: 'arraybuffer'})
          .then(response => {
                var fileURL = window.URL.createObjectURL(new Blob([response.data]));
                var fileLink = document.createElement('a');
                fileLink.href = fileURL;
                fileLink.setAttribute('download', 'segments.csv');
      ...
      exportTemplate(){
        ...
        axios.post('/api/segments/export/template',{responseType: 'arraybuffer'})
          .then(response => {
                var fileURL = window.URL.createObjectURL(new Blob([response.data]));
                var fileLink = document.createElement('a');
                fileLink.href = fileURL;
                fileLink.setAttribute('download', 'segmentstemplate.csv');
                document.body.appendChild(fileLink);
                fileLink.click();
      ...
      getTrash(){
        axios.post('api/segments/gettrash/')
          .then(response => {
      ...
      trashBulkRegister(){
        ...
        var bodyFormData = new FormData()
        bodyFormData.append('ids', this.selectedIds); 
        axios.post('/api/segments/trash/multiple', bodyFormData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        })
