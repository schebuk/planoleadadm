...
export default {
    ...
    data() {      
      ...
      let fields = [
        ...
        {name: 'adId', type:'related',table:'name',url:'/api/ads/getselect/',description:'name'},
        {name: "negReason", type: "text"}, 
        {name: "devReason", type: "text"}, 
        {name: "negDate", type:  "text"}, 
        {name: "devDate", type:  "text"}, 
        {name: "category", type:  "text"},  
        {name: "price", type:  "text"},  
        {name: "clientId", type:'related',table:'name',url:'/api/clients/getselect/',description:'name'},  
        {name: "qualityId",type:'related',table:'name',url:'/api/qualitys/getselect/',description:'name'},
        {name: "note", type: "text"},  
        {name: "font", type:  "text"},  
        {name: "segmentId", type:'related',table:'name',url:'/api/segments/getselect/',description:'name'}, 
        {name: "segmentCNPJType", type: "text"},  
        {name: "segmentPersonType", type: "text"},  
        {name: "segmentOperator", type: "text"},  
        {name: "segmentLives", type: "text"}, 
        {name: "exibitionDate", type: "text"},  
        {name: "leadTypeId", type:'related',table:'name',url:'/api/leadtype/getselect/',description:'name'},
    ...
    methods: {
      getRegisters(page = 1,url = '/api/leads/?page=') {
        this.tableData.draw++;
      ...
      getColumns(){
        axios.post('/api/config/columns/leads',{id:1})
          .then(response => {
      ...
      save(formfields,modalname){        
        ...
        if(formfields.id){
          ...
          axios.post('/api/leads/edit', values)
          .then(response => {
            this.snackbar = true
          ...
          axios.post('/api/leads/save', values)
          .then(response => {  
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      saveBulk(formfields,modalname){  
        ...
        axios.post('/api/leads/edit/multiple', bodyFormData, {
          headers: {
      ...
      changestatus(status,id){
        status = status? 1:0
        axios.get('/api/leads/status/'+id+'/'+status)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      onFileChanged(e) {
        ...
        axios.post('/api/leads/import', formData)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      exportRegisters(){
        ...
        axios.post('/api/leads/export',{responseType: 'arraybuffer'})
          .then(response => {
      ...
      exportTemplate(){
        ...
        axios.post('/api/leads/export/template',{responseType: 'arraybuffer'})
          .then(response => {
                var fileURL = window.URL.createObjectURL(new Blob([response.data]));
                var fileLink = document.createElement('a');
                fileLink.href = fileURL;
                fileLink.setAttribute('download', 'leadstemplate.csv');
                document.body.appendChild(fileLink);
                fileLink.click();
      ...
      trashBulkRegister(){
        ...
        var bodyFormData = new FormData()
        bodyFormData.append('ids', this.selectedIds); 
        axios.post('/api/leads/trash/multiple', bodyFormData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        })
