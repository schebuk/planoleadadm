...
export default {
    ...
    methods: {
      getRegisters(page = 1,url = '/api/clients/trash/list/?page=') {
        this.tableData.draw++;
      ...
      getColumns(){
        axios.post('/api/config/columns/clients',{userId:1})
          .then(response => {
      ...
      restore(id){
        axios.get('/api/clients/restore/'+id)
          .then(response => {
            this.snackbar = true
      ...
      restoreBulkRegister(){
        ...
        axios.post('/api/clients/restore/multiple', bodyFormData, {
            headers: {
