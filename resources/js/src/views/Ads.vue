  ...
  <div class="Ads">
    ...
    <div class="topButtons">
      ...
      >
        ...
        <Form :fields="fields" modalname="cad" title="Adicionar" type="create" @closemodal="closemodal" @save="save"></Form>
      </v-dialog>
      <v-btn
        class="mx-2"
        fab
...
export default {
    ...
    methods: {
      getRegisters(page = 1,url = '/api/ads/?page=') {
        this.tableData.draw++;
      ...
      getColumns(){
        axios.post('/api/config/columns/ads',{id:1})
          .then(response => {
            if (response.data.data){
              ...
              this.columns = [
                ...
                {"name": "codChannel", "type": "varchar(255)", "label": "codChannel"}, 
      ...
      save(formfields,modalname){        
        let  values = {
          ...
          codChannel: formfields.codChannel,
          status: formfields.status,
        }
        if(formfields.id){
          values['id'] = formfields.id
          axios.post('/api/ads/edit', values)
          .then(response => {
            this.snackbar = true
          ...
          axios.post('/api/ads/save', values)
          .then(response => {  
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      saveBulk(formfields,modalname){  
        ...
        axios.post('/api/ads/edit/multiple', bodyFormData, {
          headers: {
      ...
      changestatus(status,id){
        status = status? 1:0
        axios.get('/api/ads/status/'+id+'/'+status)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      onFileChanged(e) {
        ...
        axios.post('/api/ads/import', formData)
          .then(response => {
            this.snackbar = true
            this.toastText = response.data.message
            console.log(response)
      ...
      exportRegisters(){
        ...
        axios.post('/api/ads/export',{responseType: 'arraybuffer'})
          .then(response => {
                var fileURL = window.URL.createObjectURL(new Blob([response.data]));
                var fileLink = document.createElement('a');
                fileLink.href = fileURL;
                fileLink.setAttribute('download', 'ads.csv');
      ...
      exportTemplate(){
        ...
        axios.post('/api/ads/export/template',{responseType: 'arraybuffer'})
          .then(response => {
                var fileURL = window.URL.createObjectURL(new Blob([response.data]));
                var fileLink = document.createElement('a');
                fileLink.href = fileURL;
                fileLink.setAttribute('download', 'adstemplate.csv');
                document.body.appendChild(fileLink);
                fileLink.click();
      ...
      trashBulkRegister(){
        ...
        var bodyFormData = new FormData()
        bodyFormData.append('ids', this.selectedIds); 
        axios.post('/api/ads/trash/multiple', bodyFormData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        })
      ...
      deleteBulkRegister(){
        ...
        var bodyFormData = new FormData()
        bodyFormData.append('ids', this.selectedIds); 
        axios.post('/api/ads/delete/multiple', bodyFormData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        })
