...
export default {
    ...
    methods: {
      getRegisters(page = 1,url = '/api/qualitys/trash/list/?page=') {
        this.tableData.draw++;
      ...
      getColumns(){
        axios.post('/api/config/columns/qualitys',{userId:1})
          .then(response => {
      ...
      restore(id){
        axios.get('/api/qualitys/restore/'+id)
          .then(response => {
            this.snackbar = true
      ...
      restoreBulkRegister(){
        ...
        axios.post('/api/qualitys/restore/multiple', bodyFormData, {
            headers: {
